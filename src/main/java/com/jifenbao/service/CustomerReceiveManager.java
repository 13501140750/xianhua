/**
 * Copyright 2008 - 2016 Powered by OpenTide <br/>
 * This class was automatically generated with otc framework generator
 *
 */
package com.jifenbao.service;

import com.otc.framework.service.BaseManager;
import com.jifenbao.model.CustomerReceive;

/**
 *
 * @ClassName      CustomerReceiveManager
 * @Description
 * CustomerReceive
 * 业务管理类.
 * <p/>
 * 继承于BaseManager,不需任何代码即拥有默认的对CustomerReceive对象的CRUD函数.
 *
 * @author      liujing
 * @Date        2016-02-03 10:57:03
 * @version     1.0 
 *
 */

public interface CustomerReceiveManager extends BaseManager<CustomerReceive, Long> {
    /**
     * Bean名称
     */
    String BEAN_NAME = "customerReceiveManager";


}
